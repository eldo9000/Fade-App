...
### Promoted at audit-cycle close (2026-04-20)

These were surfaced during the 4-session arc but explicitly deferred as out-of-scope. Each is ready for the next cycle's triage without re-discovery.

| ID | Claim | Next step |
|---|---|---|
| **B16.2 / B19** | Async lifecycle for 14 read-only sync commands (`analyze_*`, `preview_*`, `get_waveform`, `get_spectrogram`, `diff_subtitle`, `lint_subtitle`, `probe_subtitles`, `get_file_info`, `get_streams`) — B16 phase 1 landed sentinel cleanup only; the XL async work is intact | Phase 2 of B16: convert 3–4 commands at a time, each batch as its own sub-PR. `from_result` bridge in `convert/` modules is a prerequisite for full sentinel cleanup. |
| **AudioOffset i64→i32** | `AudioOffset.offset_ms` is Rust `i64`, generated by ts-rs as `bigint` in `AudioOffset.ts`, but the frontend passes it as `number`. Precision drift for values > 2^53 ms (~285 years) — cosmetic in practice, real in type contract | Micro-patch: change field to `i32` (max offset ~24 days) or add `#[ts(type = "number")]` attribute. |
| **Windows non-C: drive preview** | `assetProtocol.scope` narrowed (B4) to `$HOME/**`, `$TEMP/**`, `/Volumes/**`, `/media/**`, `/mnt/**`. Files on Windows secondary drives (e.g. `D:\`) outside `%USERPROFILE%` are unreachable by the webview. Deferred at B4 | Implement runtime `asset_protocol_scope().allow_file(path)` expansion when user opens a file from a non-primary drive. Requires Tauri 2 runtime capability API. |
| **Slot-leak watchdog** | `createLimiter(n)` (B10, `src/lib/concurrency.js`) parks slots until a `job-done`/`job-error`/`job-cancelled` event fires. If an event fires with an unknown `jobId` or if the frontend is torn down mid-batch, resolvers are never called and the slot count drains to 0 — future conversions silently queue forever | Add a timeout-based drain (`setTimeout(resolve, 30_000)`) as a safety valve, or instrument `active`/`queued` counts into the status UI so a stuck limiter is visible. Separate concern: add a unit test for the teardown path. |
| **librewin_common superset strategy** | B5 worked around a `librewin_common` pin (`v0.1.3`) by adding a local `StoredPreset` superset that reads/writes the same JSON file. Strategy is implicit — if a future Fade field also conflicts with a pinned common type, the pattern repeats without guidance | Make the strategy explicit: document the `StoredPreset` superset pattern in `ARCHITECTURE.md`; decide whether Fade should own `fade-presets.json` schema entirely (authoritative) or remain a superset consumer (follow-along). |
| **GHA shell injection** | `release.yml:110,365` interpolate `${{ inputs.tag }}` and `${{ github.event_name }}` directly into `run:` steps. `github.event_name` is system-controlled (safe); `inputs.tag` is write-access-gated `workflow_dispatch` input — exploitable only by repo collaborators, not external. Semgrep `run-shell-injection` findings. | Low urgency but tighten anyway: assign `inputs.tag` to an env var (`TAG: ${{ inputs.tag }}`) and reference `$TAG` inside the shell. Standard hardening. |

---

## 8. Audit cycle close — 2026-04-20

**Arc:** 4 sessions, 1 day. Session 1 baseline → Session 2 concern-based → Session 3 adversarial → Session 4 (this file) execution.

**Finding disposition:**
- 33 findings in scope (20 CONFIRMED + 13 WEAKENED-kept)
- 18 PR batches planned → **18 shipped** (B1–B18 all DONE with SHAs)
- 13 dropped (see §6 Drop list)
- 9 NEEDS-EVIDENCE deferred (see §7)
- 6 new items promoted at close (see §7 Promoted section)

**Batch SHAs:**

| Batch | SHA | Summary |
|---|---|---|
| B1 | `50cbf80` | serde_yml → serde_yaml_ng |
| B2 | `cea4a39` | write_fade_log atomic append |
| B3 | `dc998e3` | IPC input caps (scan_dir, filmstrip, diff_subtitle) |
| B4 | `2a58924` | assetProtocol scope + open_url scheme allowlist |
| B5 | `c6c4254` | presets normalize_loudness + file lock |
| B6 | `01299c5` | cancel flicker + preview stale-result guard |
| B7 | `969d5b8` | seven_zip_bin memoize + RateLimiter scaffold |
| B8 | `efad2cc` | run_ffmpeg consolidation (3 copies → 1) |
| B9 | `137e4bc` | cancel TOCTOU + filmstrip cancel hook |
| B10 | `83bc4d3` | batch fanout semaphore (createLimiter) |
| B11 | `c7a9555` | JobOutcome enum + unified finalizer |
| B12 | `356c545` | waveform streaming RMS + bucket cap |
| B13 | `c70aa1a` | loudnorm 3→2 pass + double-probe collapse |
| B14 | `1f87107` | multi-extract collapse (ExtractMulti) |
| B15 | `2cb2bd6` | validate_output_name umbrella + run_operation coverage |
| B16 | `830d105` | op_result typed bridge (phase 1); async lifecycle deferred (phase 2) |
| B17 | `95e7812` | ts-rs codegen for IPC boundary |
| B18 | `72e18c4` | parking_lot::Mutex + return-shape drift + doc refresh |

**Verify pass (2026-04-20 re-run of static analysis):**
- Semgrep: 20 findings — all 20 are `temp-dir` INFO (uuid-namespaced scratch files, same as baseline). Zero new non-INFO findings in Rust or frontend code.
- cargo-audit: 19 advisories — RUSTSEC-2025-0067 (libyml) and RUSTSEC-2025-0068 (serde_yml) confirmed absent. Remaining 17 are identical to baseline (Linux-only GTK3 transitive + unmaintained build-time crates + rand edge-case). No regressions.
