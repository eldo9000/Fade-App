...
2026-04-28 | OPEN | DNxHR resolution guard bypassed at arg-builder layer — full_sweep video_full surfaces all 5 dnxhr_* cases failing on the 64×64 fixture even though convert/video.rs has the 1280×720 minimum guard. The guard lives in convert(), not in build_ffmpeg_video_args(); any direct caller of the arg builder bypasses it. Move the guard into the arg builder or document the convert()-only contract.
