...
fn stream_samples<R: Read, F: FnMut(f32)>(mut reader: R, mut sink: F) -> io::Result<()> {
...
pub fn get_waveform(
    ...
    if let Err(e) = stream_samples(stdout, |s| accum.push(s)) {
        let _ = child.kill();
        let _ = child.wait();
        return Err(format!("waveform read failed: {e}"));
    }
    let _ = child.wait();
...
mod tests {
    ...
    fn stream_samples_parses_whole_word_stream() {
        ...
        let mut out = Vec::new();
        stream_samples(Cursor::new(input), |s| out.push(s)).unwrap();
    ...
    fn stream_samples_handles_byte_boundary_splits() {
        ...
        for chunk in [1, 2, 3, 5, 6, 7, 9, 13] {
            ...
            let mut out = Vec::new();
            stream_samples(reader, |s| out.push(s)).unwrap();
            assert_eq!(
                out, samples,
    ...
    fn stream_samples_handles_empty_stream() {
        ...
        stream_samples(Cursor::new(Vec::<u8>::new()), |s| out.push(s)).unwrap();
        assert!(out.is_empty());
    }
    ...
    fn stream_plus_accumulator_end_to_end() {
        ...
        stream_samples(Cursor::new(bytes), |s| acc.push(s)).unwrap();
        let w = acc.finish();
